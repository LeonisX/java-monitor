...
* `gui.horizontalScale` и `gui.pageSize` регулируют плотность отображения данных по горизонтали. 
...
* `charts`: настройка одного или нескольких линейных графиков.
  * `id`: идентификатор графика. К нему будут привязаны отдельные характеристики. Пример: `0`, `1`, `2`, ... Допускаются строковые значения.
...
Значения `upperBound` и `tickUnit` так же можно менять динамически.
